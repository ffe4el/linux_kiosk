...
int main() {
    ...
    // 서버 소켓 생성
    if ((server_fd = socket(AF_UNIX, SOCK_STREAM, DEFAULT_PROTOCOL)) == 0) {
    ...
    // 소켓과 주소를 바인딩
    if (bind(server_fd, &serverUNIXaddr, sizeof(serverUNIXaddr)) < 0) {
        perror("bind failed");
        exit(EXIT_FAILURE);
    }
    ...
    while (1) {
        // 클라이언트의 연결 수락
        int client_socket = accept(server_fd, &clientUNIXaddr, &clientlen);
        if (client_socket < 0) {
